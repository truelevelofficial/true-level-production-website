...
export async function GET(request: NextRequest) {
  try {
    if (!isGoogleOAuthEnabled()) return NextResponse.redirect(accountUrl(request, "google_not_configured"));
    ...
    if (!tokenResponse.ok) return NextResponse.redirect(accountUrl(request, "google_token_failed"));
    const tokens = (await tokenResponse.json()) as { access_token?: string };
    if (!tokens.access_token) return NextResponse.redirect(accountUrl(request, "google_token_missing"));

    const userResponse = await fetch("https://openidconnect.googleapis.com/v1/userinfo", {
      headers: { authorization: `Bearer ${tokens.access_token}` },
    });

    if (!userResponse.ok) return NextResponse.redirect(accountUrl(request, "google_user_failed"));
    const user = (await userResponse.json()) as GoogleUser;
    if (!user.email || user.email_verified === false) return NextResponse.redirect(accountUrl(request, "google_email_unverified"));
