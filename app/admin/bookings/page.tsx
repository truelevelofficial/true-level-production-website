    ...
    <AdminShell title="Bookings">
      ...
      <div className="grid gap-4">
        {bookings.map((booking) => <form action={updateBookingAction} className="grid gap-4 rounded-[2rem] border border-[#06111F]/10 bg-white p-5 shadow-sm lg:grid-cols-6" key={booking.id}>
          ...
          <div className="lg:col-span-2"><p className="text-xs font-black uppercase text-[#0B7CFF]">{booking.type} / {booking.status}</p><h2 className="text-2xl font-black">{booking.client.fullName}</h2><p className="text-sm text-[#06111F]/55">{booking.client.companyName} {booking.client.phone}</p><p className="text-sm text-[#06111F]/55">{displayDate(booking.startTime)} - {displayDate(booking.endTime)}</p></div>
