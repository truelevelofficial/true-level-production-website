...
const kanbanStages = [
  ...
  ["MEETING", "Meeting", "bg-indigo-100 text-indigo-800"],
  ["QUOTATION", "Quotation", "bg-purple-100 text-purple-800"],
          ...
          <form action={createWorkflowProjectAction} className="mt-4 grid gap-3 md:grid-cols-4">
            ...
            <div>
              ...
              <select className={inputClass} name="stage">
                {kanbanStages.map(([v, en]) => <option key={v} value={v}>{en}</option>)}
              </select>
            </div>
            <div>
              ...
              <div key={stageId} className="flex min-w-[260px] flex-col rounded-[1.6rem] border border-[#06111F]/10 bg-white/60 p-4">
                ...
                <div className="grid flex-1 auto-rows-max gap-3 overflow-y-auto">
                  {stageProjects.length === 0 ? (
                    ...
                    <div key={p.id} className="rounded-[1.2rem] border border-[#06111F]/10 bg-white p-4 shadow-sm transition hover:shadow-md">
