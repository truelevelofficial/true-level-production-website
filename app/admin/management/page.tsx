...
const siteMedia: MediaAsset[] = [
  ...
  { key: "service-brand-films-video", path: "/videos/service-brand-films.mp4", label: "Svc Video — Brand Films", description: "Brand films background video (auto-plays muted on card)", usage: "Homepage services — Brand Films card", recommendedWidth: 1080, recommendedHeight: 1920, type: "video", group: "services", serviceTitle: "Brand Films" },
  ...
  { key: "service-creative-direction-video", path: "/videos/service-creative-direction.mp4", label: "Svc Video — Creative Direction", description: "Creative direction background video (auto-plays muted)", usage: "Homepage services — Creative Direction card", recommendedWidth: 1080, recommendedHeight: 1920, type: "video", group: "services", serviceTitle: "Creative Direction" },
  ...
  { key: "service-ugc-content-video", path: "/videos/service-ugc-content.mp4", label: "Svc Video — UGC Content", description: "UGC content background video (auto-plays muted)", usage: "Homepage services — UGC Content card", recommendedWidth: 1080, recommendedHeight: 1920, type: "video", group: "services", serviceTitle: "UGC Content" },
  ...
  { key: "service-studio-shoots-video", path: "/videos/service-studio-shoots.mp4", label: "Svc Video — Studio Shoots", description: "Studio shoots background video (auto-plays muted)", usage: "Homepage services — Studio Shoots card", recommendedWidth: 1080, recommendedHeight: 1920, type: "video", group: "services", serviceTitle: "Studio Shoots" },
  ...
  { key: "service-event-coverage-video", path: "/videos/service-event-coverage.mp4", label: "Svc Video — Event Coverage", description: "Event coverage background video (auto-plays muted)", usage: "Homepage services — Event Coverage card", recommendedWidth: 1080, recommendedHeight: 1920, type: "video", group: "services", serviceTitle: "Event Coverage" },
  ...
  { key: "service-campaign-assets-video", path: "/videos/service-campaign-assets.mp4", label: "Svc Video — Campaign Assets", description: "Campaign assets background video (auto-plays muted)", usage: "Homepage services — Campaign Assets card", recommendedWidth: 1080, recommendedHeight: 1920, type: "video", group: "services", serviceTitle: "Campaign Assets" },
...
const groupNames: Record<string, { label: string; description: string }> = {
  ...
  "services": { label: "Services Section", description: "Images upload directly (file picker). Videos: paste an external URL (YouTube, Vimeo, CDN) — stored in config" },
...
function AssetCard({ asset, currentUrl }: { asset: MediaAsset; currentUrl?: string }) {
  const info = asset.type === "video" ? null : getFileInfo(asset.path);
  return (
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_0.8fr]">
        <div>
          <div className="mb-3 flex items-center gap-3">
            ...
            {asset.type === "video" ? (
              currentUrl ? (
                ...
                <span className="rounded-full bg-amber-100 px-3 py-0.5 text-[10px] font-bold text-amber-700">No URL</span>
              )
            ) : info?.exists ? (
              <span className="rounded-full bg-green-100 px-3 py-0.5 text-[10px] font-bold text-green-700">Active</span>
            ) : (
          ...
          <div className="mt-5 rounded-2xl border border-dashed border-[#06111F]/15 bg-[#F7F8FB] p-4">
            <p className="mb-2 text-[10px] font-black uppercase tracking-[0.1em] text-[#06111F]/40">
              {asset.type === "video" ? "Video URL (YouTube, Vimeo, CDN link)" : "Upload new image"}
            </p>
            {asset.type === "video" ? (
              <form action="/api/video-url" method="POST" className="flex flex-wrap items-end gap-3">
                <input type="hidden" name="title" value={asset.serviceTitle || ""} />
                <input type="url" name="url" defaultValue={currentUrl || ""} placeholder="https://example.com/video.mp4" className="block w-72 rounded-xl border border-[#06111F]/15 bg-white px-4 py-2.5 text-sm outline-none transition focus:border-[#0B7CFF]" />
                <button type="submit" className="rounded-full bg-[#0B7CFF] px-5 py-2.5 text-xs font-black uppercase tracking-[0.12em] text-white shadow-lg shadow-blue-500/20 transition hover:bg-[#06111F]">Save URL</button>
              </form>
            ) : (
          ...
          {asset.type === "video" ? (
            currentUrl ? (
              ...
              <div className="flex flex-col items-center gap-2 p-8 text-[#06111F]/30">
                ...
                <p className="text-xs font-bold">No URL set yet</p>
                <p className="text-[10px]">Enter a video URL above</p>
              </div>
            )
          ) : info?.exists ? (
...
export default async function ManagementPage(props: { searchParams?: Promise<{ success?: string; error?: string }> }) {
  ...
  const existing = siteMedia.filter((a) => {
    if (a.type === "video") return a.serviceTitle ? Boolean(videoUrls[a.serviceTitle]) : false;
        ...
        {grouped.map(({ group, info, assets }) => (
          <section key={group}>
            ...
            <div className="grid gap-5">
              {assets.map((asset) => (
                <AssetCard key={asset.key} asset={asset} currentUrl={asset.serviceTitle ? videoUrls[asset.serviceTitle] : undefined} />
              ))}
            </div>
          </section>
        ))}
